...
void _halt_if_no_devices(uint8_t prs_addr, uint8_t rhs_addr, uint8_t als_addr);
void _report_sensor(const char *name, int r, const uint8_t *addr);

void _check_prs_id(uint8_t *addr);
void _check_rhs_id(uint8_t *addr);
void _check_als_id(uint8_t *addr);

void enumerate_sensors(uint8_t *prs_addr, uint8_t *rhs_addr, uint8_t *als_addr)
{
	puts("Enumerating sensors:\r");
	_report_sensor("SPL07-003", prs_find(prs_addr), prs_addr);
	_report_sensor("ENS210...", rhs_find(rhs_addr), rhs_addr);
	_report_sensor("APDS-9999", als_find(als_addr), als_addr);

	_halt_if_no_devices(*prs_addr, *rhs_addr, *als_addr);
}

void check_sensors(uint8_t *prs_addr, uint8_t *rhs_addr, uint8_t *als_addr)
{
	puts("\nChecking sensor IDs:\r");
	if (*prs_addr <= BUS_ADDR_MAX) _check_prs_id(prs_addr);
	if (*rhs_addr <= BUS_ADDR_MAX) _check_rhs_id(rhs_addr);
	if (*als_addr <= BUS_ADDR_MAX) _check_als_id(als_addr);
